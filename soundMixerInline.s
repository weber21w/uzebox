 ...
 */
	...
	#define MIXER_TYPE_NULL		2
	#define MIXER_TYPE_CUSTOM	3
	...
	/*
	 * Determines who owns the per-scanline audio/sync timing slot.
	 *
	 * MIXER_TYPE_VSYNC  (0)	Stock VSYNC mixer. Mixes 262 samples during each VSYNC.
	 *						Requires a 524-byte RAM buffer. (Default)
	 * MIXER_TYPE_INLINE (1)	Stock inline mixer. Mixes 1 sample each HSYNC. Does not
	 *						require a RAM buffer. Not supported by every video mode.
	 * MIXER_TYPE_NULL   (2)	No audio engine. Provides only the required sync timing
	 *						stub, so programs without sound do not allocate mixer RAM.
	 * MIXER_TYPE_CUSTOM (3)	Includes CUSTOM_MIXER_FILE instead of a stock mixer.
	 */
	#ifndef SOUND_MIXER
		#define SOUND_MIXER MIXER_TYPE_VSYNC
	#endif

	#ifndef CUSTOM_MIXER_FILE
		#define CUSTOM_MIXER_FILE "soundMixerCustom.s"
	#endif

	/*
	 * Remove the stock channel/music ABI.
	 *
	 * This omits mixer.channels, tracks[], tr1/tr2/tr3/tr4/tr5 state, steptable,
	 * default waves, and stock SetMixer/Trigger/music-player code.
	 *
	 * The null mixer implies NO_CHANNELS=1. Custom mixers also default to
	 * NO_CHANNELS=1 because most custom engines own their own channel state; set
	 * -DNO_CHANNELS=0 explicitly if a custom mixer intentionally reuses the stock
	 * channel ABI.
	 */
	#ifndef NO_CHANNELS
		#if ((SOUND_MIXER == MIXER_TYPE_NULL) || (SOUND_MIXER == MIXER_TYPE_CUSTOM))
	...
	#if ((SOUND_MIXER == MIXER_TYPE_NULL) && (NO_CHANNELS == 0))
		#error MIXER_TYPE_NULL has no stock channel backend; do not force NO_CHANNELS=0.
	#endif
	...
	#define PATCH_END		0xff
		...
		#ifndef AUDIO_OUT_HSYNC_CYCLES
			#define AUDIO_OUT_HSYNC_CYCLES 135
		#endif
		#ifndef AUDIO_OUT_VSYNC_CYCLES
			...
			#endif
			#ifndef CHANNEL_STRUCT_SIZE
				#define CHANNEL_STRUCT_SIZE 0
			#endif
		#endif

	#elif SOUND_MIXER == MIXER_TYPE_INLINE
		#define WAVE_CHANNELS		3
		#define NOISE_CHANNELS		1
		#define MIXER_CHAN4_TYPE	0
		#define CHANNEL_STRUCT_SIZE	6
			...
			#endif
		#endif
	#elif SOUND_MIXER == MIXER_TYPE_VSYNC
	...
	#else
		#error Invalid SOUND_MIXER type. Use MIXER_TYPE_VSYNC, MIXER_TYPE_INLINE, MIXER_TYPE_NULL, or MIXER_TYPE_CUSTOM.
	#endif
